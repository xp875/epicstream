...
with st.form("input_form"):
    ...
    with tab1:
        prompt = st.text_area("Describe topic to be quizzed on:")
    with tab2:
    ...
    if submit_button and prompt:
        ...
        with st.spinner("Generating quiz..."):
            response = generate_quiz(st.session_state.prompt, difficulty=difficulty, questions_number=questions_number)
            st.session_state.quiz = response.output_parsed
            st.session_state["estimated_cost"] = response.usage.input_tokens * 20 + response.usage.output_tokens * 120
            st.session_state["estimated_cost"] /= 1e6
...
if st.session_state.quiz is not None:
    ...
    st.write(f"Estimated cost: {st.session_state['estimated_cost']:.2} cents")
